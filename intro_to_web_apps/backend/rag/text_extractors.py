...
class MarkitdownExtractor(BaseTextExtractor):
    """Extractor that uses markitdown to convert various file formats to markdown."""
    ...
    def extract(self, filepath: Union[str, List[str]]) -> List[str]:
        ...
        Returns:
            A list of extracted text content, one item per document
        """
        # Convert single filepath to list for consistent processing
        if isinstance(filepath, str):
            filepath = [filepath]
        ...
        for file in filepath:
            ...
            if path.is_file():
                try:
                    ...
                    markdown_results.append(result.text_content)
                except Exception as e:
                    print(f"Error extracting from {file}: {e}, skipping...")
                    continue
            else:
                print(f"Filepath: {file} is not a file, skipping...")
                continue
