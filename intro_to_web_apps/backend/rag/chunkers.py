...
from intro_to_agents.agents.llms import BaseLLM, OPENAI_TOKEN_LIMITS
...
class SentenceChunker(BaseChunker):
    ...
    def __init__(self, window_size: int = 3):
        ...
        Args:
            window_size: Number of sentences to include in each window on either side of the current sentence.
        """
    ...
    def chunk(self, text: Union[str, List[str]]) -> List[List[str]]:
        ...
        for t in tqdm(text, desc="Chunking by sentence windows", unit="document"):
            ...
            # Use the window stored in metadata, not node.text
            chunks = [
                node.metadata["prev_text"]   # or whatever key you set
                for node in nodes
            ]
            docs.append(chunks)
